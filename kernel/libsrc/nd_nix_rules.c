...
int nd_nfm_add_bypass_rules (__u32 _saddr, __u32 _daddr)
{
        ...
        struct list_head  *pos, *next;
	printk ("nd_nfm_add_bypass_rules start..\n");
        if (!list_empty(&nd_list_bypass_rules.list))    {
		printk ("nd_nfm_add_bypass_rules bpyass list is not empty..\n");
                list_for_each_safe (pos, next, &nd_list_bypass_rules.list)      {

                        bypass_rule = list_entry (pos, struct nd_bypass_rule_data, list);
                        if (bypass_rule )
                        {
                                if (bypass_rule->saddr == _saddr && bypass_rule->daddr == _daddr)
                                {
					printk("nd_nfm_add_bypass_rules :: exist rule!!!\n");
                                        return ND_ERROR_ALREADEXIST_RULE;
                                }
                        }
                }
	...
	printk ("nd_nfm_add_bypass_rules not found exist rule... so add!!!\n");

        new_bypass_rule = kmalloc (sizeof (struct nd_bypass_rule_data), GFP_KERNEL);
        if (new_bypass_rule == NULL)
        {
                return ND_ERROR_ENOMEM;
        }
        ...
        new_bypass_rule->saddr = _saddr;
        new_bypass_rule->daddr = _daddr;
	...
	printk ("nd_nfm_add_bypass_rules :: success add!!!\n");
        ...
        if (!list_empty (&nd_list_bypass_rules.list))   {
                ...
                list_for_each_safe (pos, next, &nd_list_bypass_rules.list)      {
                        ...
                        if (bypass_rule)        {
                                ...
                                memset (&szTmp, 0, sizeof (szTmp));
                                size = snprintf (szTmp, sizeof (szTmp), "%u|%u\n", bypass_rule->saddr, bypass_rule->daddr);

                                strcat_safe (output + len , szTmp, ND_IOCTL_DATA_SIZE - len);
                                len += size;
                        }
                }
        }
