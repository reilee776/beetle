...
static long nd_device_ioctl (struct file *file, unsigned int cmd, unsigned long arg)	{
	...
	struct cmd_bypass_rule_data cmd_bypass_rule_pars;
	...
	switch (cmd)		{
		...
		case IOCTL_ADD_BYPASS_RULE:
			printk ("[RECV] ADDBYPASS RULE - bypass rule add request...\n");
			if (copy_from_user(&cmd_bypass_rule_pars, (struct cmd_bypass_rule_data *)arg, sizeof (struct cmd_bypass_rule_data)))
			{
				
				return -EFAULT;
			}

			ret = nd_nfm_add_bypass_rules (cmd_bypass_rule_pars.saddr, cmd_bypass_rule_pars.daddr);
			if (ret == 0 )
			{
				printk ("Successfully added policy - added bypass rule [saddr = %u], [daddr =%u]\n",cmd_bypass_rule_pars.saddr, cmd_bypass_rule_pars.daddr );
			}			
			
		break;

		case IOCTL_DEL_BYPASS_RULE:
			printk ("[RECV] DEL RULE - bypass rule del request...\n");
			if (copy_from_user(&cmd_bypass_rule_pars, (struct cmd_bypass_rule_data *)arg, sizeof (struct cmd_bypass_rule_data)))	
			{
			...
			ret = nd_nfm_del_bypass_rule( cmd_bypass_rule_pars.saddr, cmd_bypass_rule_pars.daddr);
			if (ret == 0 )          {
	            printk ("Successfully deleted policy - deleted bypass rule [saddr = %u], [daddr =%u]\n",cmd_bypass_rule_pars.saddr, cmd_bypass_rule_pars.daddr );
            }

		break;

		case IOCTL_GET_BYPASS_RULES:
			printk ("[RECV] GET RULE - bypass rule get request...\n");
			nd_nfm_get_bypass_rules(cmd_str_data);
		...
		case IOCTL_RESET_BYPASS_RULES:
			printk ("[RECV] RESET RULE - bypass rule reset request...\n");
			nd_nfm_reset_bypass_rule ();
			printk ("Successfully reset bypasspolicy....");
		break;
...
unsigned int local_in_hook_func(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
    ...
    // 현재 프로세스의 PID를 가져옵니다.
    pid_struct = find_get_pid(current->pid);
    if (pid_struct) {
        task = pid_task(pid_struct, PIDTYPE_PID);
        if (task) {
            printk(KERN_INFO "[inbound hook]Process name: %s, PID: %d\n", task->comm, task->pid);
        }
        put_pid(pid_struct);
    }


    return NF_ACCEPT; // 패킷을 허용
/*
	...
	struct sock *sk;
        ...
        if (!skb)
        {
                printk ("nd_nix_hook_inbound_func :: SKB NULL!!!!\n");
                return NF_ACCEPT;
        }
	...
	sk = skb->sk;
        if (!sk)
        {
                printk ("nd_nix_hook_inbound_func :: SK IS NULL!!!\n");
                return NF_ACCEPT;
        }

        rcu_read_lock();
        task = pid_task(find_vpid(sk->sk_socket->file->f_owner.pid), PIDTYPE_PID);
        rcu_read_unlock();

        if (task) {
                //pid = task->pid;
                //printk(KERN_INFO "PID of process handling this packet: %d\n", pid);
		printk (KERN_INFO "PID of process handling this packet!\n");
        } else {
                printk(KERN_INFO "Unable to retrieve PID\n");
        }

        printk ("nd_nix_hook_inbound_func :: ~~~~~~~~~\n");
*/
}
/*
 *
...
#endif
#ifdef ND_BYPASS_SD_SAME_IP
				if ( nd_nfm_chk_bypass_rule (iph->saddr, iph->daddr) == 1)
#else	
				if (    
						iph->saddr == ND_LOOPBACK &&
...
static struct nf_hook_ops nf_local_in_hook = {

	.hook		= local_in_hook_func,
	.pf		= PF_INET,
...
//	 nd_major_number = register_chrdev(NOD_MAJOR, ND_DEVICE_NAME, &fops);
	...
	//nf_register_net_hook (&init_net, &nf_local_in_hook);
...
static void __exit nd_nix_nfm_exit(void)
{
	...
	//nd_list_bypass_rules();
	nd_nfm_reset_bypass_rule();
	
	nf_unregister_net_hook (&init_net, &nf_inbound_hook);
	//nf_unregister_net_hook (&init_net, &nf_local_in_hook);
