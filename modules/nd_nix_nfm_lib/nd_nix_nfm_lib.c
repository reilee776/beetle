...
int sdk_add_NdNixNfmDrv_bypass_policy (struct cmd_bypass_rule_data* bypass )
{
        ...
        ret = ioctl (fd, IOCTL_ADD_BYPASS_RULE, bypass);
        if (ret < 0 )
        {
                close (fd);
                return -1;
...
int sdk_del_NdNixNfmDrv_bypass_policy (struct cmd_bypass_rule_data* bypass )
{
        ...
        ret = ioctl (fd, IOCTL_DEL_BYPASS_RULE, bypass);
        if (ret < 0 )
        {
                close (fd);
                return -1;
...
int sdk_get_NdNixNfmDrv_bypass_policy (char *data )
{
        ...
        ret = ioctl (fd, IOCTL_GET_BYPASS_RULES, data);
        if (ret < 0 )
        {
                close (fd);
                return -1;
...
int sdk_reset_NdNixNfmDrv_bypass_policy (void )
{
        ...
        ret = ioctl (fd, IOCTL_RESET_BYPASS_RULES, NULL);
        if (ret < 0 )
        {
                close (fd);
                return -1;
