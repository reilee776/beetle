...
unsigned long ip_to_number(const char *ip) {
    unsigned long num = 0;
    unsigned int octet;
    char *token;
...
int main(int argc, char** argv)	{
	...
	char szSessionCnt[8] = {0,};
	char log_buffer[MAX_LOGS * LOG_MSG_SIZE];
	int ret = 0;
	struct cmd_bypass_rule_data bypass = {16777343,16777343};

	struct cmd_service_rule_pars_data policy = {1,22, 1410, 1};
	struct cmd_service_sub_rule_pars_data subpolicy[] = {{22, 0, 3232300517, 0, 0},{3306, 0,3232300518,0,0},{3306, 1,32513195,32513197,0}}; 
	struct cmd_service_sub_rule_pars_data subpolecyitm = {22, 1, 32513195, 32513197, 0};

	struct cmd_service_sub_rule_pars_data subpolecy_sourceip = {22, 0, 32513196, 0, 0};
	...
	if (argc == 2 )
	{
		...
		{
			sdk_get_NdNixNfmDrv_ManagedSessionCnt(szSessionCnt);
			printf ("Now Managed Session Count: %s\n", szSessionCnt);
		}

		else if (strcmp (argv[1], "logs") == 0 )
		{
			printf ("logs call..\n");	
			sdk_get_NdNixNfmDrv_logs(log_buffer);
			printf ("%s\n", log_buffer);
		}
	}
	...
	if (argc >= 3 )
	{
		...
		if (strcmp (argv[1], "bypass") == 0 )
		{
			if (strcmp (argv[2], "add") == 0 )
			{
				...
				sdk_add_NdNixNfmDrv_bypass_policy(&bypass);
			}
			
			else if (strcmp (argv[2], "del") == 0 )
			{
				sdk_del_NdNixNfmDrv_bypass_policy(&bypass);
			}

			else if (strcmp (argv[2], "reset") == 0 )
			{
				sdk_reset_NdNixNfmDrv_bypass_policy();
			}

			else if (strcmp (argv[2], "get") == 0 )
			{
				sdk_get_NdNixNfmDrv_bypass_policy(szCmdRet);
				printf ("[bypass rule]\n%s\n", szCmdRet);
			}
		}
